...
T_SQL_Connect *SQL_Connect;
	...
	ret=DAU_mk(&t_DAU,SQL_Connect,tabname);
	if(ret) {
...
int i,j,ret,finished=0;
	...
	for(i=0;i<j;i++) {
		...
		if(ret==-1) {
        		___SQL_Transaction__(ctx->SQL_Connect,TRANROLLBACK);
			json_object_put(rqjson);
			return -1; //-1:rollback
		}
		if(ret>0) finished++;
...
/* 调用删、改、插服务 */
	...
	if(!conn->status) ___SQL_Transaction__(sp->SQL_Connect,TRANBEGIN);
	...
	nethead->ERRNO1=sp->SQL_Connect->Errno;
        if(!conn->status) {
		___SQL_Transaction__(sp->SQL_Connect,TRANCOMMIT);
//无状态
        }
