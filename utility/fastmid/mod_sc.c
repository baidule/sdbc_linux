...
//从就绪队列取一个任务
		...
		if(conn == (T_Connect *)-1) {
			conn=NULL;
			ShowLog(1,"%s:get poolno[%d] fault for TCB:%d",__FUNCTION__,poolno,TCBno);
		...
		if(ret<0) {
			...
			ShowLog(1,"%s:tid=%lu,bad bind connect to application!,TCB:%d,ret=%d",__FUNCTION__,
				tid,TCBno,ret);
			qp->tid=0;
			break;
		}
...
//*
	ShowLog(3,"%s:get pool[%d] for TCB:%d",__FUNCTION__,poolno,TCBno);
